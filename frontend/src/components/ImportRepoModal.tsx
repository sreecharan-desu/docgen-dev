...
const ImportRepoModal: React.FC<ImportRepoModalProps> = ({ isOpen, onClose, onImport, projectId }) => {
  ...
  const [filterType, setFilterType] = useState<"all" | "code" | "images">("all");
  ...
  const filteredFiles = useCallback(() => {
    ...
    return files.filter((file) => {
      ...
      if (filterType === "code") return ["js", "ts", "jsx", "tsx", "py", "java", "cpp"].includes(ext || "");
      if (filterType === "images") return ["png", "jpg", "jpeg", "gif"].includes(ext || "");
      return true;
    });
  }, [files, filterType]);
...
`;
  ...
  interface ImportRepoModalProps {
    isOpen: boolean;
    onClose: () => void;
    onImport: (url: string, type: string, source: string, files: string[]) => Promise<void>;
    projectId: string | undefined;
  }
      ...
      <ScrollArea className="h-[calc(100vh-200px)] w-64 border-r border-gray-600 bg-[#1A2525] text-gray-200">
        <div className="p-2">
          <div className="flex items-center justify-between p-2 text-xs uppercase tracking-wider text-gray-400">
            ...
            >
              {viewMode === "tree" ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
            </Button>
          </div>
          <div className="flex items-center gap-2 p-2">
            ...
            <Select
              value={filterType}
              onValueChange={(value) => setFilterType(value as "all" | "code" | "images")}
            >
    ...
    <div className="flex h-[calc(100vh-100px)] w-full bg-[#1A2525] rounded-lg overflow-hidden border border-gray-600">
      ...
      <div className="flex-1 flex flex-col">
        <div className="flex items-center justify-between p-2 bg-[#2A3A3A] border-b border-gray-600">
          ...
          <span className="text-sm font-medium text-gray-200">{selectedFolder} - Preview</span>
          <Button
            variant="ghost"
            size="sm"
        ...
        <div className="flex-1 p-6 bg-[#1A2525] text-gray-200 flex items-center justify-center">
          <div className="text-center">
            <h2 className="text-lg font-medium text-white mb-4">Repository: {selectedFolder}</h2>
  ...
  const renderFileSelection = () => (
    <div className="p-6 bg-[#2A3A3A] rounded-lg border border-gray-600">
      <div className="flex flex-col items-center justify-center p-6">
        ...
        <h3 className="text-base font-medium text-white mb-2">Select Repository Folder</h3>
        <Button
