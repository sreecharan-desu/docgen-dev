  ...
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-2 overflow-hidden rounded-md border p-4 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        ...
        success: "border-green-500/20 bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400",
        destructive: "border-red-500/20 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400",
      },
    },
    defaultVariants: {
      variant: "default",
...
const ToastIcon = ({ variant }: { variant?: "default" | "success" | "destructive" }) => {
  if (variant === "success") return <Check className="h-4 w-4" />
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      "absolute right-1 top-1 rounded-md p-1 opacity-0 transition-opacity hover:text-foreground focus:opacity-100 focus:outline-none group-hover:opacity-100",
      className
    )}
    toast-close=""
    {...props}
